          ...
          <div className="relative overflow-hidden">
           ...
           <div className="flex items-center justify-center gap-3 mt-6">
            {CaseStudiesdata.map((_, index) => (
              <button key={index} onClick={() => setCurrent(index)} className={`h-[10px] w-[10px] rounded-full transition-all ${ current === index ? "bg-[#ff8600] w-[32px] h-[12px]" : "bg-white opacity-75" }`} />
            ))}
          </div>
          </div>
        </div>
